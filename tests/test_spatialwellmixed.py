...
import pytest
from aldsim.models.spatial import SpatialParticleWellMixed
from aldsim.chem import Precursor, ALDchem


class TestSpatialParticleWellMixed:
    ...
    def test_init(self):
        ...
        model = SpatialParticleWellMixed(self.chem, self.p, self.p0, self.T, self.S, self.flow)
    ...
    def test_init_rejects_dual_pathway(self):
        ...
        with pytest.raises(NotImplementedError):
            SpatialParticleWellMixed(dual_chem, self.p, self.p0, self.T, self.S, self.flow)
    ...
    def test_run_with_tdose_and_dt(self):
        ...
        tdose = model.t0() * 2
        dt = model.t0() * 0.1
        t, cov, x = model.run(tdose=tdose, dt=dt)
        assert t.shape == cov.shape
        assert t.shape == x.shape
